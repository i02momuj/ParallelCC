 ...
 * @version 2012.02.27
 */
public class ParallelCC6 extends TransformationBasedMultiLabelLearner {
     ...
     */
    public ParallelCC6(Classifier classifier, int[] aChain) {
        super(classifier);
        chain = aChain; 
    }
     ...
     * used for training each of the binary models
     */
    public ParallelCC6(Classifier classifier) {
        super(classifier);
    }
    ...
    protected void buildInternal(MultiLabelInstances train) throws Exception {
        ...
        //At first, all labes in trained are 0
        ...
        //Create a dataset to store the predicted labels
        ...
        ArrayList<Integer> toAddLabels = new ArrayList<Integer>();
...
//            System.out.println(Arrays.toString(indicesToRemove2));
            ...
            for(int j=0; j<numLabels; j++) {
            	if(j != chain[i]) {
            		if(trained[j] == 0) { //Label has not been trained yet -> remove
            			toRemoveLabels.add(labelIndices[chain[j]]);
            		}
            		else { //If label has been trained, include predictions to dataset
            			toAddLabels.add(chain[j]);
            		}
            			
            	}
            }
            
            int [] indicesToRemove = new int[toRemoveLabels.size()];
            //ArrayList to Array
            for(int j=0; j<toRemoveLabels.size(); j++) {
            	indicesToRemove[j] = toRemoveLabels.get(j);
            }
